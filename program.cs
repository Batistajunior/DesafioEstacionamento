using System;
using diobootCamp.DesafioEstacionamento;
...
if (convert1)
{
    ...
    bool convert2 = int.TryParse(inputPrecoHora, out int entradaPorHoras);
    if (convert2)
    {
        Console.Clear();
        //criar um estacionamento com as caracteristicas inputadas pelo usuário(preco e preco por hora)
        sistemaDeEstacionamento estacionamento = new sistemaDeEstacionamento(entradaPreco, entradaPorHoras);
            ...
            if (convert3)
            {
                ...
                else if (entrada3 == 2)
                {
                    ...
                    if (estacionamento.VeiculoEstacionado(placaVeiculo))
                    {
                        ...
                        if (convert4)
                        {
                            int preco = (horas * entradaPorHoras) + entradaPreco;
                            Console.Clear();
                ...
                else if (entrada3 == 3)
                {
                    ...
                    foreach (veiculos v in estacionamento.listaVeiculos)
                    {
